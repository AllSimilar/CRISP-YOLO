...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        # ---- Dehaze auxiliary branch: load paired clear images ----
        # Configure in data.yaml with EITHER:
        #   clear_root: <dir>               # clear images share filename with hazy ones
        #   hazy2clear: ["/hazy/", "/clear/"]  # substring replacement on hazy path
        self.clear_root = self.data.get("clear_root", None)
        self.hazy2clear = self.data.get("hazy2clear", None)
        self.use_dehaze = (self.clear_root is not None) or (self.hazy2clear is not None)

    def _get_clear_path(self, hazy_path: str) -> str:
        """Map a hazy image path to its paired clear image path."""
        if self.hazy2clear is not None:
            old, new = self.hazy2clear
            return hazy_path.replace(old, new)
        if self.clear_root is not None:
            return str(Path(self.clear_root) / Path(hazy_path).name)
        return hazy_path
    ...
    def __getitem__(self, index):
        """Return label dict, with optional 'clear_img' for the dehaze auxiliary branch.

        IMPORTANT: For pixel-level supervision to be valid, the hazy image and clear image must
        remain spatially aligned. This wrapper only re-applies a deterministic LetterBox to the
        clear image, so when self.use_dehaze=True you should disable all random geometric
        augmentations in your hyp file (mosaic, mixup, cutmix, fliplr, flipud, degrees, translate,
        scale, shear, perspective all = 0.0).
        """
        label = super().__getitem__(index)
        if not self.use_dehaze:
            return label

        hazy_path = self.im_files[index]
        clear_path = self._get_clear_path(hazy_path)
        clear = cv2.imread(clear_path)
        if clear is None:
            LOGGER.warning(f"Clear image not found at {clear_path}, falling back to hazy image.")
            clear = cv2.imread(hazy_path)
        clear = cv2.cvtColor(clear, cv2.COLOR_BGR2RGB)

        # Deterministic letterbox to match label['img'] spatial size
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img", "text_feats", "sem_masks", "clear_img"}:
