...
class Converse2D(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 5,
                 scale: int = 1, padding: int = 4, padding_mode: str = 'circular',
                 eps: float = 1e-5):
        super().__init__()
...
class Decoder(nn.Module):
    ...
    def __init__(self, ch: tuple = (256, 512, 1024), c2: int = 3,
                 up_to_input: bool = True, converse_final: bool = False):
        super().__init__()
        c_p3, c_p4, c_p5 = ch
        ...
        self.up_1 = Converse2D(
            c_p4, c_p4,
            kernel_size=5, scale=2, padding=4,
            padding_mode='circular', eps=1e-3,
        )
        ...
        if not up_to_input:
            ...
            self.upsample = Converse2D(
                c_mid, c_mid,
                kernel_size=5, scale=4, padding=4,
                padding_mode='circular', eps=1e-3,
            )
        else:
...
class TransposeDecoder(nn.Module):
    ...
    def __init__(self, ch: tuple = (256, 512, 1024), c2: int = 3,
                 up_to_input: bool = True, transpose_final: bool = False):
        super().__init__()
        c_p3, c_p4, c_p5 = ch
        self.up_to_input = up_to_input
        ...
        self.up_1 = nn.ConvTranspose2d(c_p4, c_p4, kernel_size=3, stride=2,
        ...
        self.up_2 = nn.ConvTranspose2d(c_p3, c_p3, kernel_size=3, stride=2,
                                       padding=1, output_padding=1)
        ...
        self.up_3 = nn.ConvTranspose2d(c_mid, c_mid, kernel_size=3, stride=2,
                                       padding=1, output_padding=1)

        self.upsample = nn.Upsample(scale_factor=4, mode='bilinear',
                                    align_corners=True) if up_to_input else nn.Identity()
